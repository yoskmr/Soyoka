...
flowchart TB
    ...
    RecordComplete -->|"きおくを見るタップ"| MemoDetail
    RecordComplete -.->|"自動クローズ\n（ホームに留まる）"| Home
...
### 14.5 AI整理 待機体験の改善（2026-06-10追記）

> AI整理が「ことばを整えています…」のまま固まる問題（02-ai-pipeline.md 5.6 参照）の
> UI側対策と、保存後フローのホーム滞在型への変更。

#### 処理中ストール検知と中断導線（メモ詳細画面）

処理ステータスが30秒進展しない場合のみ、処理中アニメーションの下に中断導線を表示する:
...
- 通常時（30秒未満）は表示しない（静かな待機体験を維持）
- 「中断する」タップで即座に待機状態（idle）へ戻す。キュー側から届く
  「キャンセルされました」failed 通知は失敗として表示せず idle に読み替える
  （ユーザー操作による中断を失敗扱いしない）
- 中断後は「AI整理を実行する」ボタンからやり直せる

#### 処理中の文字起こしプレビュー（メモ詳細画面）

AI整理の待ち時間中も、処理中アニメーションの下に文字起こしテキスト（本人のことば）を
最大8行プレビュー表示する。
UX原則「画面は完成を待たず、表示できる部分から順に見せる」準拠。

#### 録音完了トーストのライフサイクル（ホーム滞在型）

保存1.5秒後にメモ詳細へ自動遷移する従来フローを廃止し、ホームに留まる。
連続キャプチャを優先する（UX原則「1つの操作のたびに構える必要がある体験にしない」）。

| タイミング | 挙動 |
|:----------|:-----|
| 保存直後 | 「書きとめました」+「ことばを整えています…」（段階表示） |
| AI整理完了時 | 「整えました」に切替 → 2秒後にトーストを自動クローズ |
| AI整理が長引く場合 | 最大10秒でトーストを自動クローズ（整理はバックグラウンドで継続） |
| 「きおくを見る」タップ | メモ詳細へ遷移（唯一の遷移経路） |
| 背景タップ | トーストを閉じる |

- AI整理失敗時はトーストを「整えました」にせず、リトライ導線はメモ詳細画面に委ねる
- タイミング定数: `RecordingFeature.completionToastMaxDuration`（10秒）/
  `completionToastLingerDuration`（2秒）
