...
### 2026-06-10: 完了トーストのホーム滞在型への変更 + 待機体験の改善

「ことばを整えています…」のまま固まる問題の修正に伴い、以下を変更（正式仕様は
`docs/spec/ai-voice-memo/design/04-ui-design-system.md` 14.5 および
`02-ai-pipeline.md` 5.6 を参照）:

- **トーストのライフサイクル**: 保存後の自動遷移（1.5秒でメモ詳細へ）を廃止し、
  ホームに留まる。AI整理完了「整えました」の2秒後に自動クローズ。
  長引く場合は最大10秒でクローズし、整理はバックグラウンドで継続
- **AI処理完了通知の中継**: AppReducer は `state.recording.aiProcessingCompleted` の
  直接変更ではなく、子アクション `recording(.aiProcessingCompleted)` として中継する
  （クローズタイマー制御を RecordingFeature に集約するため）
- **失敗時の扱い**: AppReducer の監視ループは `.failed` 受信でも終了する
  （トーストは「整えました」にせず、リトライ導線はメモ詳細画面に委ねる）
- **メモ詳細の待機体験**: ステータスが30秒進展しない場合の中断導線
  （「いつもより時間がかかっています…」+「中断する」）と、
  処理中の文字起こしプレビュー表示を追加
